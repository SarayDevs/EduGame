...
func _physics_process(_delta):
	...
	if Input.is_action_just_pressed("Atacar"):
		...
		$Ata/CollisionShape2D2.disabled = false
		Anim.visible = false
		spr.visible = true
		$AtacarAnimacion.play("Attack")
		ataque.play()
...
func _on_recolectar_area_entered(area):
	if area.is_in_group("item"):
		collectedItems += 1
		recolectar.play()
		if collectedItems >= 5:
			print("Felicidades Ganaste")
			emit_signal("MaxItems")
	if area.is_in_group("puerta"):
		$"..".Fin()

func _on_atacar_animacion_animation_finished(anim_name):
	match anim_name:
		"Attack":
			$Ata/CollisionShape2D2.disabled = true
			Anim.visible = true
			spr.visible = false

func _on_ata_body_entered(body):
